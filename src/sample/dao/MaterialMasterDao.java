 ...
 *
 */
public class MaterialMasterDao extends Dao {
	/**
	 * コンストラクタ。
	 * @param obj JDBC接続可能オブジェクト。
	 * @throws Exception 例外。
	 */
	public MaterialMasterDao(final JDBCConnectableObject obj) throws Exception {
		super(obj);
	}
	 ...
	 */
	public static class MaterialMasterTableQuery extends Query {
		/**
		 * コンストラクタ。
		 */
		public MaterialMasterTableQuery() {
	...
	public static FieldList getQueryResultFieldList() {
		Query query = new MaterialMasterTableQuery();
	...
	public int insert(final Map<String, Object> data) throws Exception {
		return this.executeInsert(new MaterialMasterTable(), data);
	}
...
//		return this.executeRemove(new MaterialMasterTable(), data); // レコードの論理削除(DeleteFlagの設定)
		return this.executeDelete(new MaterialMasterTable(), data); // レコードの物理削除
	}
}
