...
public class Test3Query extends Query {
	...
	/**
	 * TestQueryの副問合せを取得します。
	 * @return TestQueryの副問合せ。
	 */
	 ...
	 */
	public Test3Query() {
		this.setComment("テスト3");
		this.setDistinct(false);
		this.testSubQuery = new TestSubQuery();

		this.setFieldList(new FieldList(
			this.testSubQuery.getTestQuery().getMaterialIdField()
			, this.testSubQuery.getTestQuery().getMaterialCodeField()
			, this.testSubQuery.getTestQuery().getMaterialNameField()
			, this.testSubQuery.getTestQuery().getMaterialUnitField()
			, this.testSubQuery.getTestQuery().getUnitPriceField()
			, this.testSubQuery.getTestQuery().getOrderPointField()
			, this.testSubQuery.getTestQuery().getMemoField()
			, this.testSubQuery.getTestQuery().getUnitPrice2Field()
		));
